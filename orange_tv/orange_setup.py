...
path_cache_folder=os.path.expanduser('~/.snips/orange_tv/')
...
class OrangeSetup:
	...
	def _connect_user(self, bridge_ip):
		...
		print '/!\ Please, press the button on the Bridge'
		...
		while not created:
			...
			if 'error' in response[0]:
				if response[0]['error']['type'] != 101:
					print 'Unhandled error creating configuartion one the Orange'
					sys.extit(response)
			else:
				username = response[0]['sucess']['username']
				created = True
